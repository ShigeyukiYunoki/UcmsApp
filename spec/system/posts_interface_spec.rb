  ...
  scenario 'image only jpeg,gif and png' do
    ...
    aggregate_failures do
      expect do
        fill_in 'タイトル', with: 'title'
        fill_in '投稿内容', with: 'content'
        attach_file 'post[image]', Rails.root.join('spec/factories/1292866.svg')
        click_button '投稿'
  ...
  scenario 'image less than 5MB' do
    ...
    aggregate_failures do
      page.accept_confirm('アップロード可能なファイルサイズは5MBまでです。より小さいサイズのファイルを選んでください。') do
        fill_in 'タイトル', with: 'title'
        fill_in '投稿内容', with: 'content'
        attach_file 'post[image]', Rails.root.join('spec/factories/1296_autumn_kouyou_momiji_sky_9948.jpg')
      end
